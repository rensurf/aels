  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [
        ...
        "arn:aws:dynamodb:ap-southeast-2:${data.aws_caller_identity.current.account_id}:table/aels-phrase-groups",
        "arn:aws:dynamodb:ap-southeast-2:${data.aws_caller_identity.current.account_id}:table/aels-phrase-groups/index/*",
      ]
    }]
  })
}
